        ...
        foreach (var row in reader)
        {
            ...
            var transaction = new TexansFinancialTransaction
            {
                TransactionSignature =
                    ReadColumnValue(row, headers.IndexOf("Transaction ID")).ToString(),
                TransactionDate =
                    DateTimeOffset.ParseExact(
                        ReadColumnValue(row, headers.IndexOf("Effective Date"))
                            .ToString(), "M/d/yyyy",
                        CultureInfo.InvariantCulture),
                PostingDate =
                    DateTimeOffset.ParseExact(
                        ReadColumnValue(row, headers.IndexOf("Posting Date")).ToString(),
                        "M/d/yyyy",
                        CultureInfo.InvariantCulture),
                TransactionType =
                    ReadColumnValue(row, headers.IndexOf("Transaction Type")).ToString(),
                Amount =
                    decimal.Parse(
                ...
                Category =
                    ReadColumnValue(row, headers.IndexOf("Transaction Category"))
                        .ToString(),
                ...
                ExtendedDescription =
                    ReadColumnValue(row, headers.IndexOf("Extended Description"))
                        .ToString()
            };
